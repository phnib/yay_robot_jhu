...
class SequenceDataset(torch.utils.data.Dataset):
    ...
    def get_phase_history(self, selected_phase_demo_dict, sorted_phases, curr_ts):
        ...
        if self.phase_history_only_phase_switches_flag:
            ...
            first_phase_partially_stitched_episode = list(selected_phase_demo_dict.keys())[0]
            min_episode_idx = 0 # Note: Will be < 0 for the new precessor phases
            remainin_precessor_phases_reversed = sorted_phases[:sorted_phases.index(first_phase_partially_stitched_episode)][::-1]
            selected_phase_demo_dict_with_pre_phases = selected_phase_demo_dict.copy()
            for precessor_phase in remainin_precessor_phases_reversed:
    ...
    def __getitem__(self, index):
        ...
        if total_len_wo_p2 < required_num_samples: # Check that the stitched partial episode is long enough (for all input data)
            print(f"Total length of the demos is too short: {total_len_wo_p2} with phase 1 ({phase_1}): {len_phase_1}.")
        ...
        # TODO: Check if this is a good solution for min_phase_1_phase_2_idx
        ...
        if selected_phase_demo_dict[phase_1]["phases_demo_end_idx"] >= self.history_len * self.history_step_size:
            min_phase_1_phase_2_idx = self.history_len * self.history_step_size
        else:
            min_phase_1_phase_2_idx = selected_phase_demo_dict[phase_1]["phases_demo_end_idx"]
        ...
        phase_1_phase_2_sampling_std = max_phase_1_phase_2_idx / (2*3)
...
if __name__ == "__main__":
    ...
    history_len = 3
    prediction_offset = 10 # Get command for the current timestep
